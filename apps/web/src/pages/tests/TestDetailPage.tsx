...
export const TestDetailPage: React.FC = () => {
  ...
  const { data: testData, isLoading } = useTest(id!);
  const test: any = testData;
  ...
  const marksColumns = [
    ...
    { key: 'rank', header: 'Batch Rank', render: (m: any) => {
      const rank = (test as any).rankings.find((r: any) => r.studentId === m.studentId)?.batchRank;
    ...
    <div id="test-detail-page">
      ...
      <div className="card overflow-hidden">
        <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center flex-wrap gap-4">
          <h3 className="font-semibold text-gray-900">Student Marks ({(test as any).marks.length})</h3>
          <div className="flex gap-2">
            {test.status !== 'PUBLISHED' && (
              <>
                <button onClick={() => navigate(`/tests/${test.id}/marks`)} className="btn-secondary btn-sm gap-2">
                  <Edit3 className="h-4 w-4" /> {((test as any).marks?.length || 0) > 0 ? 'Edit Marks' : 'Enter Marks'}
                </button>
                <button 
                  onClick={() => computeMutation.mutate(test.id)} 
                  disabled={!test || (test as any).marks?.length === 0 || computeMutation.isPending}
